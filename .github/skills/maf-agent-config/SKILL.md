...
tools:
  ...
  - hosted: file_search                 # GA hosted Foundry tool
...
context_providers:
  - type: foundry_memory
    ...
    scope: "{{ runtime.user_id }}"      # bound per call from authenticated identity
...
- `hosted:` entries map to `FoundryChatClient` factory methods. Non-GA hosted tools require an
  explicit `stage: preview` key, so shipping one is a visible diff.
- `context_providers` are constructed by the loader, not the agent module.
...
topology: maf_bridge          # foundry_agent | maf_bridge | hosted
agent: concierge              # -> config/agents/concierge.agent.yaml

connection:
  endpoint: ${AZURE_VOICELIVE_ENDPOINT}
  model: gpt-realtime         # omit when topology: foundry_agent
...
Select the overlay with one variable (`APP_PROFILE=prod`). Do not fork whole files per
...
## Publishing to Foundry (topology A)

`config/agents/*.agent.yaml` is the **source of truth**. A Foundry prompt agent definition is a
**build artifact** generated from it:

```text
concierge.agent.yaml  --build_agent()-->  Agent  --to_prompt_agent()-->  create_version()
```

Never hand-edit the agent in the portal and also keep the YAML. Publish from the repo in CI and
treat portal edits as drift. Record the published version back into the overlay
(`agent_version:`) so a deployment pins rather than floats.
...
3. Copy a voice profile; set `topology`, `agent`, and the voice.
...
| `topology: foundry_agent` alongside `session.instructions` | Contradictory; agent definition wins |
