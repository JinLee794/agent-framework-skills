...
| `topology` | enum | yes | `foundry_agent` \| `maf_bridge` \| `hosted` |
| `agent` | str | conditional | Agent config stem. Required for `maf_bridge` |
| `foundry_agent` | mapping | conditional | Required for `topology: foundry_agent` |
| `connection` | mapping | yes | Transport and model selection |
...
|---|---|---|
| `foundry_agent` | `foundry_agent.name`, `foundry_agent.project` | `session.instructions`, `session.tools`, `connection.model` |
| `maf_bridge` | `agent`, `connection.model`, `session.interim_response` | `foundry_agent` |
| `hosted` | `agent` | `connection`, `session` (the host negotiates both) |

`foundry_agent` forbids `session.instructions`/`session.tools` because the Foundry agent
definition is authoritative — sending them anyway is misleading dead config.

`maf_bridge` requires `interim_response` because the caller would otherwise sit in silence
while the local MAF agent runs.

## `foundry_agent` (topology A)

```yaml
foundry_agent:
  name: ${FOUNDRY_AGENT_NAME}
  project: ${FOUNDRY_PROJECT_NAME}       # bare project name, NOT the endpoint URL
  version: ${FOUNDRY_AGENT_VERSION:-}    # omit to float on latest; pin in prod
```

These become flattened kwargs on `connect(agent_name=..., project_name=..., agent_version=...)`.

## `connection`

| Key | Maps to | Notes |
|---|---|---|
| `endpoint` | `connect(endpoint=)` | `${AZURE_VOICELIVE_ENDPOINT}` |
| `model` | `connect(model=)` | Omit for `topology: foundry_agent` |
...
Credential is never in YAML. The loader supplies `DefaultAzureCredential` /
`ManagedIdentityCredential` from `settings`.
...
| `mcp` | `MCPTool` — requires `url`, `label`, and `approval: always` for side effects |
