...
visible on the next iteration of the function-calling loop.
...
Compose local sources directly instead of subclassing `SkillsProvider`: `FileSkillsSource`,
...
| `.cursor/skills/` | the coding agent working on this repo | no |

Same format, different consumers. Never point `SkillsProvider` at `.cursor/skills/`.
...
- A tool that calls another cloud service violates this seed's two-resource contract. Search
  tools may call only the configured Azure AI Search service.
...
| `SkillsProvider` pointed at `.cursor/skills/` | Build-time skills shipped to runtime |
